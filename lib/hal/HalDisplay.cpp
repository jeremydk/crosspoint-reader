...
void HalDisplay::begin(bool seamless) {
  std::unique_ptr<Panel> panel = gpio.deviceIsX3()
                                     ? std::unique_ptr<Panel>(makeUniqueNoThrow<X3Panel>())
                                     : std::unique_ptr<Panel>(makeUniqueNoThrow<X4Panel>());
  if (!panel) {
    LOG_ERR("HAL", "OOM: Panel");
    return;
  }
  einkDisplay.emplace(std::move(panel), EPD_SCLK, EPD_MOSI, EPD_CS, EPD_DC, EPD_RST, EPD_BUSY);
