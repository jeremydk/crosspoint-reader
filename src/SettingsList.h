...
inline std::vector<SettingInfo> getSettingsList(const SdCardFontRegistry* registry = nullptr) {
  ...
  // Plugin-contributed web settings. Each plugin appends its own SettingInfo
  // rows (credential fields, toggles, etc.) using the existing helpers.
  for (size_t i = 0; i < PluginRegistry::count(); ++i) {
    const PluginManifest* m = PluginRegistry::all()[i];
    if (m && m->appendWebSettings) m->appendWebSettings(v);
  }
  return v;
}
